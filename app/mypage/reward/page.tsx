...
export default async function RewardListPage() {
  ...
  const rewards = db.prepare("SELECT * FROM rewards WHERE visible = 1").all() as Reward[];
    ...
    <div className="pb-24 max-w-md mx-auto bg-[#F4F4F4]">
      ...
      <div className="px-4 py-5">
        ...
        <div className="flex flex-col gap-3">
          {rewards.map((r) => {
            const canExchange = user.points >= r.required_points && r.stock > 0;
            return (
              <Link
                key={r.id}
                ...
                <div className="flex-1 p-4 flex items-center gap-4">
                  ...
                  <div className="text-right shrink-0">
                    <p className={`font-black text-sm ${canExchange ? "text-[#C9A96E]" : "text-gray-300"}`}>
                      {r.required_points.toLocaleString()}P
                    </p>
                    {!canExchange && (
